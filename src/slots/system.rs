...
pub fn register(engine: &mut Engine) {
    ...
    engine.register(
        ...
        Arc::new(|_engine, ctx, node, scope| {
            ...
            let app_state = ctx.get::<Arc<crate::AppState>>("app_state")
                .map(|s| s.clone())
                .ok_or_else(|| Diagnostic { r#type: "error".to_string(), message: "AppState not found in Context".to_string(), filename: node.filename.clone(), line: node.line, col: node.col, slot: Some("system.cron_sync".to_string()) })?;
            let db_manager = app_state.db_manager.clone();

            let sync_res = tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    if let Some(crate::db::DbPool::Sqlite(pool)) = db_manager.get_pool("default").await {
                        let jobs = sqlx::query_as::<_, (String, String)>(
                            "SELECT schedule, command FROM cron_jobs WHERE is_active = 1"
                        )
                        .fetch_all(&pool)
                        .await;
                        ...
                        match jobs {
                            Ok(list) => {
                                ...
                                content.push_str("# Auto-generated by ZenoPanel. Do not edit.\n");
                                for (sched, cmd) in list {
                                    content.push_str(&format!("{} root {}\n", sched, cmd));
                                }
                                if let Err(e) = std::fs::write("/etc/cron.d/zenopanel", content) {
                                    eprintln!("[Cron] Sync failed writing crontab file: {}", e);
                                    false
                                } else {
                                    true
                                }
                            ...
                            Err(e) => {
                                eprintln!("[Cron] Sync failed querying database: {}", e);
                                false
                            }
                        }
                    } else {
            ...
            scope.set(&target, Value::Bool(sync_res));
            Ok(())
        }),
        SlotMeta { description: "".to_string(), example: "".to_string(), inputs: HashMap::new(), required_blocks: Vec::new(), value_type: "".to_string() }
    );
