...
try:
    ...
    # 2. Get /login to get _csrf cookie and csrf token
    r_get = session.get("http://localhost:8080/login")
    print("GET /login status:", r_get.status_code)
    ...
    r_post = session.post("http://localhost:8080/login", json={"username": "admin", "password": "admin"}, headers=headers)
